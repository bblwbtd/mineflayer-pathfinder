...
function inject(bot) {
  ...
  function monitorMovement() {
    ...
    const blockTime = performance.now() - lastNodeTime;
    if (!passingDoor && blockTime > 1000) {
      const blockInFront = bot.blockAtCursor(1);
      const blockUnderFoot = bot.blockAt(new Vec3(p.x, p.y, p.z));
      const nextBlock = bot.blockAt(
        new Vec3(nextPoint.x, nextPoint.y, nextPoint.z)
      );

      nearDoors = [blockInFront, blockUnderFoot, nextBlock]
        .reduce((p, c) => p.concat(c, c), [])
        .filter((v) => !!v && isWoodenDoor(v));
    ...
    if (passingDoor) {
      if (
        Math.floor(lastPlace.x) != Math.floor(p.x) ||
        Math.floor(lastPlace.z) != Math.floor(p.z) ||
        !nearDoors.length
      ) {
        passingDoor = false;
        nearDoors = [];
