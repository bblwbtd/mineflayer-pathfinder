...
class Movements {
  constructor(bot) {
    ...
    if (registry.blocksByName.cobweb)
      this.blocksToAvoid.add(registry.blocksByName.cobweb.id);
    if (registry.blocksByName.web)
      this.blocksToAvoid.add(registry.blocksByName.web.id);
    ...
    if (registry.blocksByName.cave_air)
      this.replaceables.add(registry.blocksByName.cave_air.id);
    if (registry.blocksByName.void_air)
      this.replaceables.add(registry.blocksByName.void_air.id);
    ...
    registry.blocksArray.forEach((block) => {
      if (
        this.interactableBlocks.has(block.name) &&
        ...
        for (let y = minY; y < maxY; y++) {
          for (let x = minX; x < maxX; x++) {
            for (let z = minZ; z < maxZ; z++) {
              this.entityIntersections[`${x},${y},${z}`] =
                this.entityIntersections[`${x},${y},${z}`] ?? 0;
  ...
  getBlock(pos, dx, dy, dz) {
    const b = pos
      ? this.bot.blockAt(new Vec3(pos.x + dx, pos.y + dy, pos.z + dz), false)
      : null;
    if (!b) {
      return {
        replaceable: false,
    ...
    b.safe =
      (b.boundingBox === "empty" ||
        b.climbable ||
        this.carpets.has(b.type) ||
        (b.name.includes("door") && !b.name.includes("iron"))) &&
      !this.blocksToAvoid.has(b.type);
  ...
  safeToBreak(block) {
    ...
    if (this.dontMineUnderFallingBlock) {
      ...
      if (
        this.getBlock(block.position, 0, 1, 0).canFall ||
    ...
    return (
      block.type &&
      !this.blocksCantBreak.has(block.type) &&
      this.exclusionBreak(block) < 100
    );
  }
  ...
  getMoveJumpUp(node, dir, neighbors) {
    ...
    if (blockA.physical && this.getNumEntitiesAt(blockA.position, 0, 1, 0) > 0)
      return; // Blocks A, B and H are above C, D and the player's space, we need to make sure there are no entities that will fall down onto our building space if we break them
    if (blockH.physical && this.getNumEntitiesAt(blockH.position, 0, 1, 0) > 0)
      return;
    if (
      blockB.physical &&
      !blockH.physical &&
      !blockC.physical &&
      this.getNumEntitiesAt(blockB.position, 0, 1, 0) > 0
    )
      return; // It is fine if an ent falls on B so long as we don't need to replace block C
  ...
  getMoveForward(node, dir, neighbors) {
    ...
    if (
      ...
      toPlace.push({
        ...
        dz: 0,
        useOne: true,
      }); // Indicate that a block should be used on this block not placed
    } else {
  ...
  isWoodenDoor(block) {
    return block?.name.includes("door") && !block?.name.includes("iron");
  }
  ...
  getMoveDiagonal(node, dir, neighbors) {
    ...
    for (const b of [block0, blockB1, blockC1, blockB2, blockC2]) {
      if (this.isWoodenDoor(b)) {
        return;
      }
    }
    ...
    if (y === 1) {
      ...
      this.getBlock(node, dir.x, -2, dir.z).physical ||
      blockD.liquid
    ) {
      if (!blockD.safe) return; // don't self-immolate
      cost +=
        this.getNumEntitiesAt(blockC.position, 0, -1, 0) * this.entityCost;
      neighbors.push(
        new Move(
          blockC.position.x,
          blockC.position.y - 1,
  ...
  getMoveDropDown(node, dir, neighbors) {
    ...
    if (!blockLand) return;
    if (
      !this.infiniteLiquidDropdownDistance &&
      node.y - blockLand.position.y > this.maxDropDown
    )
      return; // Don't drop down into water
    ...
    cost +=
      this.getNumEntitiesAt(blockLand.position, 0, 0, 0) * this.entityCost; // add cost for entities
  ...
  getMoveParkourForward(node, dir, neighbors) {
    ...
    for (let d = 2; d <= maxD; d++) {
      ...
      if (blockC.safe)
        cost +=
          this.getNumEntitiesAt(blockC.position, 0, 0, 0) * this.entityCost;
      ...
      if (ceilingClear && blockB.safe && blockC.safe && blockD.physical) {
        ...
        if (blockA.safe && d !== 4) {
          ...
          cost +=
            this.getNumEntitiesAt(blockB.position, 0, 0, 0) * this.entityCost;
          neighbors.push(
            new Move(
              blockB.position.x,
              blockB.position.y,
        ...
        if (blockE.physical) {
          ...
          cost +=
            this.getNumEntitiesAt(blockD.position, 0, 0, 0) * this.entityCost;
          neighbors.push(
            new Move(
              blockD.position.x,
              blockD.position.y,
